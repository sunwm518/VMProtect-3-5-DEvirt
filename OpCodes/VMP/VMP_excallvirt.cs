...
{
    class External_Callvirt : IOpCode
    {
        public override bool is_opcode(MethodDef md)
        {
            ...
            if (md.Body.Instructions[4].GetLdcI4Value() == 1)
            {
                return true;
            }
            return false;
        ...
        public override Vika_Instruction restore_instruction(BinaryReader read, Vika_Method md, Vika_Context vika)
        {
           ...
           var params_t = new TypeSig[params_count];
            ...
            if (!vika.all_devirts.ContainsKey(one + z))
            {
                ...
                MethodDefUser meth = new MethodDefUser("_ex_Callvirt_" + offset.ToString(), MethodSig.CreateStatic(rt, params_t), MethodImplAttributes.IL, MethodAttributes.Public | MethodAttributes.HideBySig);
