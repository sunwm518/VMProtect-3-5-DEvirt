    ...
    class LdcI4 : IOpCode
    {
        public override bool is_opcode(MethodDef md)
        {
            ...
            if (old)
            {
                ...
                if (add_oper.ReturnType.ElementType == ElementType.I4)
                    return true;
                else
                {
                    return false;
            ...
            else
            {
                ...
                if (add_oper.Contains("System.Int32"))
                {
        ...
        {
            var val = read.ReadInt32();
            vika.logger.debug("Disasm key: " + val);
            vika.value_stack.Push(val);
            return new Vika_Instruction(OpCodes.Ldc_I4, val);
        }
